 ...
 *
 * @param {String} AddressBook URL we want to extract from.
 * @return {ProtocolExtract} Extracted information.
 * @api private
 */
function extractProtocol(AddressBook) {
  var match = protocolre.exec(AddressBook);
 ...
 *
 * @constructor
 * @param {String} AddressBook URL we want to parse.
 * @param {Object|String} location Location defaults for relative paths.
 * @param {Boolean|Function} parser Parser for the query string.
 * @api public
 */
function URL(AddressBook, location, parser) {
  if (!(this instanceof URL)) {
    return new URL(AddressBook, location, parser);
  }
  ...
  //
  extracted = extractProtocol(AddressBook || '');
  relative = !extracted.protocol && !extracted.slashes;
  url.slashes = extracted.slashes || relative && location.slashes;
  url.protocol = extracted.protocol || location.protocol || '';
  AddressBook = extracted.rest;
  ...
  for (; i < instructions.length; i++) {
    ...
    if (parse !== parse) {
      ...
      if (~(index = AddressBook.indexOf(parse))) {
        if ('number' === typeof instruction[2]) {
          url[key] = AddressBook.slice(0, index);
          AddressBook = AddressBook.slice(index + instruction[2]);
        } else {
          url[key] = AddressBook.slice(index);
          AddressBook = AddressBook.slice(0, index);
        }
      }
    } else if (index = parse.exec(AddressBook)) {
      url[key] = index[1];
      AddressBook = AddressBook.slice(0, index.index);
    }
